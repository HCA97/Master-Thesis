...
# test dataset
parser = argparse.ArgumentParser("LPIPS Testing Script")
parser.add_argument(
...
for i, (param, deformation) in enumerate(zip(params, deformations)):
    for j, p in enumerate(param):
        ...
        lpips_ = loss(imgs1, imgs2, normalize=True).mean().item()
        print(
            f"Deformation {deformation} - Param {p} - LPIPS Score {lpips_}")
        lpips_scores[deformation].append(lpips_)
...
# plot fid
plt.figure()
for i, deformation in enumerate(lpips_scores):
    plt.subplot(2, 2, i+1)
    plt.title(f"LPIPS Score {deformation}")
    y = [0] + lpips_scores[deformation]
