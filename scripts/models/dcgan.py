...
class GAN(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if len(self._act_real) < self._n_samples:
            act = np.squeeze(vgg16_get_activation_maps(
                real, layer_idx=33, device="cpu", normalize_range=(-1, 1)).numpy())
            self._act_real = np.concatenate(
                (self._act_real, act), axis=0) if len(self._act_real) > 0 else act

        # append fake images or noise vectors
        if len(self.gen_input) < self._n_samples:
            if self.hparams.gen_model in ["unet", "refiner"]:
                ...
                self.gen_input = th.normal(
                    0, 1, (self._n_samples, self.generator.latent_dim), device=self.device)
    ...
    def on_epoch_end(self):
        ...
        if (self.current_epoch + 1) % 5 == 0 or self.current_epoch == 0:
            self.generator.eval()
            with th.no_grad():
                act_fake = np.zeros_like(self._act_real)

                for i in range(self._n_samples // self._act_samples):
                    # start and end index
                    si = i * self._act_samples
                    ei = min((i+1)*self._act_samples, self._n_samples)
            ...
            # # compute fid
            fid = fid_score(self._act_real, act_fake, skip_vgg=True)

            # logging
            self.log("loss/fid", fid)
