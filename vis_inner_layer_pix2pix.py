...
def plot_generator_steps_u_net(img, pl_module, save_dir):
    ...
    def plot_layer(out, layer_i, save_dir):
...
def plot_generator_steps(z, pl_module, save_dir):
    ...
    for i, layer in enumerate(pl_module.generator.conv_blocks):
        x = layer(x)
        # if i == 8:
        #     import torch.nn as nn
        #     x = nn.MaxPool2d(3, stride=1, padding=1)(x)
        #     x = -nn.MaxPool2d(3, stride=1, padding=1)(-x)
        #     x = nn.AvgPool2d(3, stride=1, padding=1)(x)
        #     from scripts import LRN
        #     x = LRN(local_size=3)(x)
        #     x = -nn.MaxPool2d(3, stride=1, padding=1)(-x)
        #     x = nn.MaxPool2d(3, stride=1, padding=1)(x)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Show inner layers of Pix2Pix")
    parser.add_argument("save_dir", help="save directory path")
    parser.add_argument("checkpoint_dir", help="path to Pix2Pix checkpoint")
    parser.add_argument(
    ...
    for i in range(1, n_samples+1):
        real_img, fake_img = next(iter(potsdam.train_dataloader()))
